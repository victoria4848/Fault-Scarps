...
f1_unique_fault_planes <- function(tab, grad_combos,profile_half_width,downtoside,min_min_gradient2,max_fault_width){
  ...
  for (i in 1:length(new_fp_grad_choices)){ # For unique gradient choices
    ...
    okp[[i]][[1]]           <- f1_find_fault_plane(tab,high,mid,min_fp_grad,downtoside)
    okp[[i]][[2]]           <- i
  ...
  for (i in 1:length(okp)){
    if (sum(okp[[i]][[1]])==0){
      ...
      okp[[i]][[1]]         <- f1_find_fault_plane(tab2,high,mid,min_fp_grad2,downtoside)
  ...
  for (i in 1:num_dif_fault_planes){
    ...
    if (fault_plane_found){
      ...
      if (num_fault_plane_sections>1){
        okp_ss[[i]][[1]]    <- f1_remove_multi_fault_planes(tab,okp_ss[[i]][[1]],profile_half_width,num_fault_plane_sections)

offset_from_crest_base_algorithm_new <- function(crest_base,
                                                 profiles, exclude
){
  ...
  # Find crest and base not for NA values of crest/base
  ...
  if (nrow(cr_ba_in)>0){
    ...
    for (i in 1:nrow(cr_ba_in)){
      ...
      plwi_f <- test_plwi[which(plane1.residuals==min(plane1.residuals))]  #best plane 
      ...
      plwi_f <- test_plwi[which(plane2.residuals==min(plane2.residuals))]  #best plane 

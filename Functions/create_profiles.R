create_profiles <- function(fault_name,profile_spacing,profile_point_spacing,profile_half_width){
  
  # Load fault and cropped DEM
  fault <- readRDS(paste0('Faults_R/',fault_name,'.RDS'))
  fault.df    <- readRDS(paste0('Faults_R/',fault_name,'.df.RDS'))
  fault.df.sm    <- readRDS(paste0('Faults_R/',fault_name,'.df.sm.RDS')) # To assign distances to profiles 
  dem_crop <- readRDS(paste0('DEMs_R/',fault_name,'_dem_crop.RDS'))
  ...
  dist_along_fault <- data.frame(prof_ind=1:length(prof_dists_overall_unlist),dist_along_fault=prof_dists_overall_unlist)

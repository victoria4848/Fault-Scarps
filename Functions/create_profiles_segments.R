create_profiles_segments <- function(fault_name,mov_av,profile_spacing,profile_half_width){
  ...
  for (f in (1:num_fault_segs)){
    ...
    for (i in 1) {
      ...
      transects <- SpatialLines(list(Lines(list(Line(cds)), ID = "one")),proj4string = crs(l.c))
      
      e <- raster::extract(l.c, transects,cellnumbers = T,method='bilinear',buffer=2)
      line1 <- data.frame(coordinates(l.c)[e[[1]][,1],])
      line1$height <- e[[1]][,2]
      names(line1) <- c('lon','lat','height')
      #line1 <- line1[order(line1$lon,line1$lat),] # left of fault always first
      ## choose smoothing ratio
      line1a <- dist4plot_UTM(line1,mov_av) 
      line1a <- line1a[order(line1a$DistTotal),] # least distance first
      rownames(line1a) <- 1:nrow(line1a)
      prof_list[[i]] <- line1a
    }
